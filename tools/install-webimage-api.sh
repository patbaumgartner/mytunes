...
# The version must match the pom's graalvm.version, so it is read from there unless overridden;
# a mismatch would leave the build resolving an artifact that does not exist.
VERSION="${1:-$(sed -n 's:.*<graalvm.version>\(.*\)</graalvm.version>.*:\1:p' pom.xml)}"
if [[ -z "$VERSION" ]]; then
	echo "Could not determine the GraalVM version from pom.xml; pass it as the first argument." >&2
	exit 1
fi
WORK="$(mktemp -d)"
